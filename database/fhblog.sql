...
CREATE TABLE `fh_article` (
  ...
  `status` smallint(1) DEFAULT NULL COMMENT '文章状态：0草稿，1博文',
  `is_top` smallint(1) DEFAULT b'0' COMMENT '置顶1，加精2，正常0',
...
CREATE TABLE `fh_category` (
  ...
  `type` smallint(1) DEFAULT NULL COMMENT '类型(1分类,2标签)',
  `status` smallint(1) DEFAULT b'0' COMMENT '状态，0正常,1删除',
